...
const Settings = () => {
  ...
  useEffect(() => {
    const newValue = JSON.stringify(notificationsEnabled);
    localStorage.setItem('settings_notifications', newValue);
  ...
  const commitRate = (raw) => {
    ...
    const newValue = JSON.stringify(rounded);
    localStorage.setItem(RATE_STORAGE_KEY, newValue);
