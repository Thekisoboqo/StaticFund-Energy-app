...
const Insights = ({ devices }) => {
    // Basic calculation for total load (kWh)
    const totalLoad = devices.reduce((acc, device) => acc + (device.watts * (device.hours || 0)), 0) / 1000;
    // Estimate bill based on $0.15 per kWh, multiplied by 30 days
    const estBill = Math.round(totalLoad * 0.15 * 30) || 0;
            ...
            <div className="content">
                ...
                <div style={{ display: 'flex', justifyContent: 'center', margin: '2rem 0', position: 'relative' }}>
                    <div style={{
                        ...
                        height: '200px',
                        borderRadius: '50%',
                        background: `conic-gradient(var(--accent) 0% 35%, var(--text-brand) 35% 65%, var(--bg-blue) 65% 100%)`,
                        display: 'flex',
                        alignItems: 'center',
                        ...
                        <div style={{
                            ...
                            <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)' }}>({totalLoad.toFixed(1)} kWh/day)</div>
                        </div>
                    </div>
