...
const Insights = ({ devices }) => {
    const [rate] = useState(() => {
        ...
        return savedRate ? parseFloat(savedRate) : 0.15;
    });

    // Calculate total load in kWh and multiply by the local electricity rate
    const totalLoad = devices.reduce((acc, device) => acc + (device.watts * (device.hours || 0)), 0) / 1000;
    const estBill = (totalLoad * 30 * rate).toFixed(2); // estimated monthly bill
            ...
            <div className="content">
                ...
                <div style={{ display: 'flex', justifyContent: 'center', margin: '2rem 0', position: 'relative' }}>
                    <div style={{
                        ...
                        height: '200px',
                        borderRadius: '50%',
                        background: 'conic-gradient(var(--accent) 0% 35%, var(--bg-blue) 35% 65%, var(--text-secondary) 65% 100%)',
                        display: 'flex',
                        alignItems: 'center',
