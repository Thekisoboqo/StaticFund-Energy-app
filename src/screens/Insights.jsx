...
const Insights = ({ devices, settings }) => {
    const totalLoad = devices.reduce((acc, device) => acc + (device.watts * (device.hours || 0)), 0) / 1000; // in kWh per day
    const monthlyLoad = totalLoad * 30; // approx per month
    const estBill = (monthlyLoad * (settings?.rate || 0.15)).toFixed(2); // Dynamic calculation
            ...
            <div className="content">
                ...
                <div style={{ display: 'flex', justifyContent: 'center', margin: '2rem 0', position: 'relative' }}>
                    <div style={{
                        ...
                        height: '200px',
                        borderRadius: '50%',
                        background: 'conic-gradient(var(--bg-card) 0% 35%, var(--accent) 35% 65%, var(--border) 65% 100%)',
                        display: 'flex',
                        alignItems: 'center',
                ...
                <div style={{
                    ...
                    <div className="card" style={{ minWidth: '160px', flex: '0 0 auto', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        ...
                        <div style={{ marginTop: 'auto', background: 'var(--bg-mint)', color: 'var(--accent)', border: '1px solid var(--border)', padding: '0.25rem 0.5rem', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 600, textAlign: 'center' }}>
                            Save ~$20/mo
                        </div>
                    </div>
                ...
                <div className="card" style={{ display: 'flex', gap: '1rem', alignItems: 'center', marginTop: '1rem' }}>
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '0.25rem' }}>
                            Your Ideal Solar Setup: {settings?.inverterSize || 5}kW System + 10kWh Battery. Invest for lifetime savings.
                        </div>
