...
const Insights = ({ devices, electricityRate = 0.15 }) => {
    // Calculate total load in kWh and use it to calculate the estimated monthly bill
    const totalLoad = devices.reduce((acc, device) => acc + (device.watts * (device.hours || 0)), 0) / 1000;
    const estBill = Math.round(totalLoad * electricityRate * 30); // Monthly bill calculation
