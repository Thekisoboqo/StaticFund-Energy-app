...
import { Clock, Thermometer, Lightbulb, Battery, Sun, ChevronRight } from 'lucide-react';

const Insights = ({ devices, settings }) => {
    // Calculate total daily load in kWh
    const totalLoad = devices.reduce((acc, device) => acc + (device.watts * (device.hours || 0)), 0) / 1000;
    // Calculate estimated monthly bill using dynamic settings
    const estBill = Math.round(parseFloat(settings.baseCost) + (totalLoad * parseFloat(settings.electricityRate)) * 30);
                ...
                <div className="card" style={{ display: 'flex', gap: '1rem', alignItems: 'center', marginTop: '1rem' }}>
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '0.25rem' }}>
                            Your Ideal Solar Setup: {settings.inverterSize}kW System + {settings.batterySize}kWh Battery. Invest for lifetime savings.
                        </div>
