...
const Insights = ({ devices, rate = 0.15 }) => {
    const estBill = useMemo(() => {
        const totalLoadKWh = devices.reduce((acc, device) => acc + (device.watts * (device.hours || 0)), 0) / 1000;
        return Math.round(50 + (totalLoadKWh * rate) * 30);
    }, [devices, rate]);
