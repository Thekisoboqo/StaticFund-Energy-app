...
const Insights = ({ devices, rate }) => {
    ...
    const totalLoadDaily = devices.reduce((acc, device) => acc + (device.watts * (device.hours || 0)), 0) / 1000;
    const estBill = Math.round(totalLoadDaily * rate * 30); // Monthly estimate
            ...
            <div className="content">
                ...
                <div style={{ display: 'flex', justifyContent: 'center', margin: '2rem 0', position: 'relative' }}>
                    <div style={{
                        ...
                        height: '200px',
                        borderRadius: '50%',
                        background: 'conic-gradient(var(--accent) 0% 35%, var(--bg-blue) 35% 65%, var(--bg-mint) 65% 100%)',
                        display: 'flex',
                        alignItems: 'center',
                    ...
                    <div style={{ position: 'absolute', top: '10%', right: '-10px', textAlign: 'left' }}>
                        <div style={{ fontWeight: 600 }}>Highest Load</div>
                        <div style={{ fontWeight: 700 }}>
                            {devices.length > 0
                                ? devices.reduce((prev, current) =>
                                    (prev.watts * prev.hours > current.watts * current.hours) ? prev : current
                                ).name
                                : 'N/A'
                            }
                        </div>
                ...
                <div className="card" style={{ display: 'flex', gap: '1rem', alignItems: 'center', marginTop: '1rem' }}>
                    <div style={{ position: 'relative' }}>
                        ...
                        <div style={{ position: 'absolute', bottom: -5, right: -5, background: 'var(--bg-primary)', borderRadius: '50%', padding: '2px' }}>
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        <button style={{ marginTop: '0.5rem', fontSize: '0.75rem', fontWeight: 700, textDecoration: 'underline', color: 'var(--accent)' }}>Learn More</button>
                    </div>
                </div>
            </div>
        </div>
