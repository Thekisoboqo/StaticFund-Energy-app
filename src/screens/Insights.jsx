...
const Insights = ({ devices, settings }) => {
    // Make use of totalLoad to calculate an estimated bill
    const totalLoad = devices.reduce((acc, device) => acc + (device.watts * (device.hours || 0)), 0) / 1000;
    // Assume 30 days and use configured rate
    const rate = settings?.rate || 0.20;
    const estBill = Math.round(totalLoad * 30 * rate) || 150; // fallback to 150
