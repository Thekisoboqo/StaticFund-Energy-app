...
const Insights = ({ devices, settings }) => {
    // Basic dynamic calculation using settings and devices (or fallback)
    const dailyKwh = devices.reduce((acc, device) => acc + (device.watts * (device.hours || 0)), 0) / 1000;
    const electricityRate = settings?.electricityRate || 0.15;
    const calculatedBill = (dailyKwh * 30 * electricityRate).toFixed(2);
    const estBill = dailyKwh > 0 ? calculatedBill : 150; // Fallback to hardcoded demo if no usage
        ...
        <div>
            ...
            <div className="content">
                ...
                <div style={{
                    ...
                    <div className="card" style={{ minWidth: '160px', flex: '0 0 auto', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            <div style={{ background: 'var(--bg-red)', padding: '0.5rem', borderRadius: '50%' }}>
