...
const Insights = ({ devices, settings }) => {
    // Calculate total load in kWh per day, then multiply by 30 for monthly, then by electricity rate
    const totalLoad = devices.reduce((acc, device) => acc + (device.watts * (device.hours || 0)), 0) / 1000;
    const estBill = (totalLoad * 30 * (settings?.electricityRate || 0.15)).toFixed(2);
            ...
            <div className="content">
                ...
                <div style={{ display: 'flex', justifyContent: 'center', margin: '2rem 0', position: 'relative' }}>
                    ...
                    <div style={{ position: 'absolute', top: '10%', right: '-10px', textAlign: 'left' }}>
                        ...
                        <div style={{ fontWeight: 700, color: 'var(--text-brand)' }}>35%</div>
                    </div>
                    <div style={{ position: 'absolute', bottom: '20%', right: '-10px', textAlign: 'left' }}>
                        <div style={{ fontWeight: 600 }}>Cooking</div>
                        <div style={{ fontWeight: 700, color: 'var(--text-brand)' }}>30%</div>
                    </div>
                ...
                <div className="card" style={{ display: 'flex', gap: '1rem', alignItems: 'center', marginTop: '1rem' }}>
                    ...
                    <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: '0.75rem', color: 'var(--text-secondary)', marginTop: '0.25rem' }}>
                            Your Ideal Solar Setup: {(settings?.inverterCapacity || 5000) / 1000}kW System + 10kWh Battery. Invest for lifetime savings.
                        </div>
