...
const Insights = ({ devices }) => {
    // Basic dynamic estimation based on tracked devices
    const getRate = () => {
    ...
    const ratePerKwh = getRate();
    const totalDailyKwh = devices.reduce((acc, device) => acc + (device.watts * (device.hours || 0) / 1000), 0);
    const estMonthlyBill = (totalDailyKwh * 30 * ratePerKwh).toFixed(2);
                ...
                <div style={{ display: 'flex', justifyContent: 'center', margin: '2rem 0', position: 'relative' }}>
                    <div style={{
                        ...
                        <div style={{
                            ...
                            <div style={{ fontSize: '1.5rem', fontWeight: 800 }}>${estMonthlyBill}</div>
                        </div>
                    </div>
                </div>
                ...
                <div className="card" style={{ display: 'flex', gap: '1rem', alignItems: 'center', marginTop: '1rem' }}>
                    <div style={{ position: 'relative' }}>
                        ...
                        <div style={{ position: 'absolute', bottom: -5, right: -5, background: 'var(--bg-card)', borderRadius: '50%', padding: '2px' }}>
                            <Battery size={12} color="var(--text-primary)" />
                        </div>
                    </div>
                    <div style={{ flex: 1 }}>
