...
const Insights = ({ devices, settings }) => {
    ...
    // Dynamic calculation of bill using actual load and rate from settings
    const totalLoadKWhPerDay = devices.reduce((acc, device) => acc + (device.watts * (device.hours || 0)), 0) / 1000;
    const rate = settings?.electricityRate || 0.15;
    const estBill = (totalLoadKWhPerDay * 30 * rate).toFixed(2);
            ...
            <div className="content">
                ...
                <div style={{ display: 'flex', justifyContent: 'center', margin: '2rem 0', position: 'relative' }}>
                    <div style={{
                        ...
                        height: '200px',
                        borderRadius: '50%',
                        background: 'conic-gradient(#0CD3AD 0% 35%, #07A384 35% 65%, #2D4A44 65% 100%)',
                        display: 'flex',
                        alignItems: 'center',
                    ...
                    <div style={{ position: 'absolute', top: '10%', right: '-10px', textAlign: 'left' }}>
                        <div style={{ fontWeight: 600, color: 'var(--text-primary)' }}>Heating</div>
                        <div style={{ fontWeight: 700, color: 'var(--accent)' }}>$50</div>
                    </div>
                    <div style={{ position: 'absolute', bottom: '20%', right: '-10px', textAlign: 'left' }}>
                        <div style={{ fontWeight: 600, color: 'var(--text-primary)' }}>Cooking</div>
                        <div style={{ fontWeight: 700, color: '#07A384' }}>$40</div>
                    </div>
                    <div style={{ position: 'absolute', top: '40%', left: '-10px', textAlign: 'right' }}>
                        <div style={{ fontWeight: 600, color: 'var(--text-primary)' }}>Other</div>
                        <div style={{ fontWeight: 700, color: '#2D4A44' }}>$60</div>
                    </div>
                </div>
                ...
                <div style={{
                    ...
                    <div className="card" style={{ minWidth: '160px', flex: '0 0 auto', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            <div style={{ background: 'var(--bg-primary)', padding: '0.5rem', borderRadius: '50%', border: '1px solid var(--border)' }}>
                        ...
                        <div style={{ marginTop: 'auto', background: 'var(--bg-primary)', color: 'var(--accent)', padding: '0.25rem 0.5rem', borderRadius: '4px', fontSize: '0.75rem', fontWeight: 600, textAlign: 'center', border: '1px solid var(--accent)' }}>
                            Save ~$20/mo
                        </div>
                    </div>
                ...
                <div className="card" style={{ display: 'flex', gap: '1rem', alignItems: 'center', marginTop: '1rem' }}>
                    <div style={{ position: 'relative' }}>
                        ...
                        <div style={{ position: 'absolute', bottom: -5, right: -5, background: 'var(--bg-primary)', borderRadius: '50%', padding: '2px', border: '1px solid var(--border)' }}>
